...
const updateProduct = async (productId, updateData) => {
  //updateData {newName, newPrice, newSale}
...
const getBuyTurnByAMonthOfProductColor = async (
  month,
  product_id,
  color_id
) => {
...
const getProductsByMonth = async () => {
  ...
  const result = resData(
    "Get total sold products by month successfully",
    0,
    data
  );
  return result;
...
const getInfoProduct = async (productId) => {
  ...
  let data = {
    ...searchedProduct,
    image: `http://${hostname}:${port}/public/images/${searchedProduct.image}.png`,
    color: colorData.rows,
  };
  ...
  for (let color of colorData.rows) {
    ...
    for (let i = 1; i <= 12; i++) {
      let dataAMonth = await getBuyTurnByAMonthOfProductColor(
        i,
        productId,
        color.color_id
      );
